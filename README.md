...
첫 실행 알림 폭탄, 한글 경계에서 `\b`가 동작하지 않던 정규식 버그,
마감 당일 리마인드가 쿨다운에 막혀 누락되던 버그 등.
...
- (수정됨) `pickReminders`의 쿨다운(`REMIND.cooldownDays=2`)이 마감 당일까지
  막아버리는 구멍이 있었다. D-1에 리마인드가 나가면 D-0(마감 당일)은
  쿨다운에 걸려 스킵되고, 다음 시도 시점(D+1)은 이미 `dday < 0`이라
  영구히 알림을 못 받았다. "지금 안 하면 못 나간다"는 리마인드의 존재
  이유를 정확히 그 대상(마감 당일)에서 놓치는 셈이었다. 마감 당일은
  쿨다운과 무관하게 항상 보내도록 예외를 추가해 고쳤다
