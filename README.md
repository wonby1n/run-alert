...
이 원칙 때문에 생기는 사각지대가 하나 있다: **이미 접수중인 채로 상태가
안 바뀌는 대회는 영원히 알림에 안 뜬다.** 신규/오픈전환/마감임박 중 어느
것도 아니기 때문이다. 그렇다고 접수중 목록 전체를 매번 나열하면 3.3의
원칙과 충돌한다.

절충안: 알림을 어차피 보내는 날(신규·오픈·리마인드 중 하나라도 있는 날)
맨 아래에 "📋 현재 접수중인 대회 N건 — 전체 목록: `SITE_URL`" 한 줄만
붙인다. 존재는 알리되 상세 나열은 화면으로 넘긴다 — 완전히 조용한 날에
이 줄만을 위해 알림을 새로 만들지는 않는다.
